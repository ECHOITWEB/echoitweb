...
export default function ESGGeneratePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        toast({
          title: '성공',
          description: 'ESG 게시물이 성공적으로 생성되었습니다.',
        });
        router.push('/admin/esg');
      } else {
        throw new Error(data.message || 'ESG 게시물 생성에 실패했습니다.');
      }
    } catch (error: any) {
      toast({
        title: '오류',
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">새 ESG 게시물 작성</h1>
      </div>
