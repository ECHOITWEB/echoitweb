...
NewsPostSchema.pre('save', async function(next) {
  ...
  if (this.isNew || this.isModified('slug')) {
    ...
    if (!this.slug || /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/.test(this.slug)) {
      ...
      const baseText = this.title.en || this.title.ko || 'news';
      this.slug = generateSafeSlug(baseText);
      
      // 슬러그가 짧은 경우 타임스탬프 추가
      if (this.slug.length < 5) {
        this.slug = `${this.slug || 'news'}-${Date.now()}`;
      }
    }

    // 단순 키워드 슬러그에 타임스탬프 추가 (예: 'ai', 'iot' 등)
    ...
    try {
      ...
      const existingPost = await NewsPost.findOne({ slug: this.slug, _id: { $ne: this._id } });
...
// 조회수 증가 메서드
NewsPostSchema.methods.incrementViewCount = function() {
  this.viewCount = (this.viewCount || 0) + 1;
  return this.save();
};

// NewsPost 모델이 이미 있는지 확인하고 없으면 생성
