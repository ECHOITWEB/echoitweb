// 더미 뉴스 데이터를 MongoDB로 마이그레이션하는 스크립트
import { connectToDatabase } from '@/lib/db/connect';
import NewsPostModel from '@/lib/db/models/NewsPost';
import { AuthorDepartment, NewsCategory } from '@/lib/db/models/NewsPost';
...
// 더미 뉴스 데이터 (src/lib/models/news-posts.ts에서 가져옴)
const dummyNewsPosts = [
  {
    id: '1',
    title: {
      ko: '에코아이티, 한컴그룹과 AI 사업 협력 MOU 체결',
      en: 'ECHOIT Signs AI Business Cooperation MOU with Hancom Group'
    },
    slug: 'hancom-ai-partnership',
    date: '2025-03-15',
    excerpt: {
      ko: '에코아이티는 AI 기술 분야에서 한컴그룹과 전략적 파트너십을 체결하여 혁신적인 AI 솔루션을 공동 개발할 예정입니다.',
      en: 'ECHOIT has established a strategic partnership with Hancom Group in the field of AI technology to jointly develop innovative AI solutions.'
    },
    content: {
      ko: `에코아이티는 AI 기술 분야에서 한컴그룹과 전략적 파트너십을 체결하여 혁신적인 AI 솔루션을 공동 개발할 예정입니다.
...
ECHOIT's CEO stated, "This collaboration with Hancom Group will be an important opportunity to accelerate business innovation using AI technology," adding, "We will combine the expertise and technical capabilities of both companies to provide better value to customers."`
    },
  ...
  },
  {
    ...
    excerpt: {
      ...
      en: 'ECHOIT has been awarded the grand prize at the 2024 Digital Innovation Award in recognition of its successful implementation of a digital transformation project in the financial sector.'
    },
    content: {
      ko: `에코아이티가 금융권 디지털 전환 프로젝트의 성공적 수행을 인정받아 2024 디지털 혁신 어워드에서 최우수상을 수상했습니다.
...
ECHOIT's project manager stated, "This award is a recognition of our efforts to create real business value through digital innovation," adding, "We will continue to do our best to successfully support our customers' digital transformation."`
    },
    category: 'award',
  ...
  },
  {
    ...
    title: {
      ...
      en: 'ECHOIT Expands Management Infrastructure with New Branch Office'
    },
    slug: 'new-branch-office-opening',
    date: '2025-01-25',
    excerpt: {
      ko: '에코아이티가 부산에 신규 지사를 설립하여 지방 고객 지원 강화 및 지역 인재 채용을 확대합니다.',
      en: 'ECHOIT establishes a new branch office in Busan to strengthen support for regional customers and expand recruitment of local talent.'
    },
    content: {
      ko: `에코아이티가 부산에 신규 지사를 설립하여 지방 고객 지원 강화 및 지역 인재 채용을 확대합니다.
...
The CEO stated, "The establishment of the Busan branch is an important investment to simultaneously achieve the two goals of balanced regional development and customer service improvement," adding, "We will continue to pursue a region-centered growth strategy."`
  ...
  },
  {
    ...
    excerpt: {
      ...
      en: 'ECHOIT has launched "EchoInsight", a next-generation AI business intelligence solution that supports data-driven decision making for enterprises.'
    },
    content: {
      ko: `에코아이티는 기업의 데이터 기반 의사결정을 지원하는 차세대 AI 비즈니스 인텔리전스 솔루션 "EchoInsight"를 출시했습니다.
...
The CTO explained, "EchoInsight is designed to enable all members of an enterprise to maximize the value of data," adding, "Through this, organizations will be able to innovate their data utilization culture and strengthen their competitiveness."`
  ...
  },
  {
    ...
    excerpt: {
      ...
      en: 'ECHOIT has signed a smart factory solution partnership with Siemens for digital transformation in the manufacturing industry.'
    },
    content: {
      ko: `에코아이티는 제조 산업의 디지털 전환을 위해 Siemens와 스마트 팩토리 솔루션 파트너십을 체결했습니다.
...
"This partnership is an ideal cooperation model that combines the strengths of Siemens, a global leader in industrial automation, and ECHOIT, a domestic IT service specialist," the CEO said, adding, "We hope to contribute to strengthening the competitiveness of domestic manufacturing through this."`
    },
    category: 'partnership',
...
// 마이그레이션 함수
export async function migrateNewsPosts() {
  try {
    // DB 연결
    ...
    // 기존 데이터 확인
    const existingCount = await NewsPostModel.countDocuments();
    console.log(`기존 뉴스 게시물 수: ${existingCount}`);

    // 중복 방지를 위한 슬러그 확인
    console.log('기존 슬러그 확인 중...');
    const existingSlugs = new Set(
      (await NewsPostModel.find({}, { slug: 1 })).map(post => post.slug)
    );
    ...
    for (const dummyPost of dummyNewsPosts) {
      try {
        // 슬러그 중복 확인
        ...
        switch (dummyPost.category) {
          ...
          case 'award':
            ...
            break;
          case 'partnership':
            category = NewsCategory.COMPANY; // 파트너십은 회사 뉴스로 간주
            break;
          case 'product':
        ...
        const newPost = {
          ...
          category,
          author: {
            department: AuthorDepartment.ADMIN, // 기본 작성자 부서
            name: '관리자'
          },
        ...
        // MongoDB에 저장
        const savedPost = await NewsPostModel.create(newPost);
        console.log(`뉴스 게시물 저장 성공: ${savedPost.slug}`);
    ...
    // 결과 보고
    console.log('\n=== 마이그레이션 결과 ===');
    console.log(`총 더미 데이터: ${dummyNewsPosts.length}`);
    ...
    // 최종 데이터 확인
    const finalCount = await NewsPostModel.countDocuments();
    console.log(`최종 뉴스 게시물 수: ${finalCount}`);
    ...
    console.error('뉴스 게시물 마이그레이션 중 오류 발생:', error);
    return {
      success: false,
